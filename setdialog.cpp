...
#define SETDIALOG_GENERALBTN_SELECTED_STYLE     "QToolButton{background-color:rgb(47,94,178);color:white;border:1px solid rgb(0, 26, 73);border-top-left-radius: 3px;border-bottom-left-radius: 3px;}"
#define SETDIALOG_GENERALBTN_UNSELECTED_STYLE   "QToolButton{background-color:white;color:rgb(47,94,178);border:1px solid rgb(0, 26, 73);border-top-left-radius: 3px;border-bottom-left-radius: 3px;}"
#define SETDIALOG_SAVEBTN_SELECTED_STYLE        "QToolButton{background-color:rgb(47,94,178);color:white;border:1px solid rgb(0, 26, 73);border-top-right-radius: 3px;border-bottom-right-radius: 3px;}"
#define SETDIALOG_SAVEBTN_UNSELECTED_STYLE      "QToolButton{background-color:white;color:rgb(47,94,178);border:1px solid rgb(0, 26, 73);border-top-right-radius: 3px;border-bottom-right-radius: 3px;}"
...
//    ui->titleLabel->setPixmap(QPixmap(":/pic/cplpic/titleBg3.png"));
    ...
    ui->lockTimeSpinBox->setValue(QString::number(CTC::getInstance()->lockMinutes).toInt());
    ...
    ui->minimizeCheckBox->setChecked( CTC::getInstance()->minimizeToTray);
    ui->closeCheckBox->setChecked( CTC::getInstance()->closeToMinimize);
    ui->resyncCheckBox->setChecked( CTC::getInstance()->resyncNextTime);
...
void SetDialog::on_saveBtn_clicked()
{
    ...
    CTC::getInstance()->lockMinutes = ui->lockTimeSpinBox->value();
    CTC::getInstance()->configFile->setValue("/settings/lockMinutes", CTC::getInstance()->lockMinutes);
    if( ui->nolockCheckBox->isChecked())
    {
        CTC::getInstance()->notProduce = false;
    }
    else
    {
        CTC::getInstance()->notProduce = true;
    }
    CTC::getInstance()->configFile->setValue("/settings/notAutoLock", !CTC::getInstance()->notProduce);
    ...
    {
        CTC::getInstance()->configFile->setValue("/settings/language", "English");
        CTC::getInstance()->language = "English";
    }


    CTC::getInstance()->minimizeToTray = ui->minimizeCheckBox->isChecked();
    CTC::getInstance()->configFile->setValue("/settings/minimizeToTray", CTC::getInstance()->minimizeToTray);

    CTC::getInstance()->closeToMinimize = ui->closeCheckBox->isChecked();
    CTC::getInstance()->configFile->setValue("/settings/closeToMinimize", CTC::getInstance()->closeToMinimize);

    CTC::getInstance()->resyncNextTime = ui->resyncCheckBox->isChecked();
    CTC::getInstance()->configFile->setValue("/settings/resyncNextTime", CTC::getInstance()->resyncNextTime);


    mutexForConfigFile.unlock();

    qDebug() << "fee: " << QString::number(ui->feeLineEdit->text().toFloat() * CTC::getInstance()->assetInfoMap.value(0).precision);
    CTC::getInstance()->postRPC(
                toJsonFormat(
...
void SetDialog::on_confirmBtn_clicked()
{
    ...
    CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_check_passphrase", "wallet_check_passphrase", QStringList() << ui->oldPwdLineEdit->text() ));
...
void SetDialog::jsonDataUpdated(QString id)
{
    if( id == "id_wallet_check_passphrase")
    {
        ...
        {
            CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_change_passphrase", "wallet_change_passphrase", QStringList() << ui->oldPwdLineEdit->text() <<  ui->newPwdLineEdit->text() ));
        }
        else
        {
