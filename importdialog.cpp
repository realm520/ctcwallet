...
void ImportDialog::on_importBtn_clicked()
{
    ...
    if( ui->privateKeyLineEdit->text().endsWith(".key") )
    {
        ...
        if( file.open(QIODevice::ReadOnly))
        {
            ...
            CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key", "wallet_import_private_key", QStringList() << str));
            ui->importBtn->setEnabled(false);
            shadowWidget->show();
        ...
        if(importEnterPwdDialog.pop())
        {
            ...
            if( file.open(QIODevice::ReadOnly))
            {
                ...
                if( output.startsWith("privateKey="))
                {
                    ...
                    CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key", "wallet_import_private_key", QStringList() << pk));
                    ui->importBtn->setEnabled(false);
                    shadowWidget->show();
                }
                else
    ...
    else // 如果输入框中是 私钥
    {
        ...
        {
            CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key", "wallet_import_private_key", QStringList() << privateKey));
...
void ImportDialog::jsonDataUpdated(QString id)
{
    if( id == "id_wallet_import_private_key")
    {
        ...
        if( result.mid(0,9) == "\"result\":")
        {
            ...
            CTC::getInstance()->needToScan = true;  // 导入成功，需要scan
            ...
            else
            {
                ...
                int size = CTC::getInstance()->addressMap.size();
                ...
                CTC::getInstance()->registerMapInsert( name, "UNKNOWN");
...
//                rpcThread->start();
                ...
                CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_scan_transaction", "wallet_scan_transaction", QStringList() << "ALL" << "true" ));
...
//                emit accepted();
        ...
        else if( result.mid(0,8) == "\"error\":")
        {
            ...
            {
                CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key_" + name, "wallet_import_private_key", QStringList() << privateKey << name << "true"));
                shadowWidget->show();
                return;
            }
            else // 如果输入框中是 私钥
            {
                CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key_" + name, "wallet_import_private_key", QStringList() << ui->privateKeyLineEdit->text() << name << "true" ));
                shadowWidget->show();
            }
        }
...
bool ImportDialog::accountNameAlreadyExisted(QString name)
{
    ...
    foreach (str, keys)
    {
        if( CTC::getInstance()->configFile->value(str) == name)
        {
            existed = true;
            break;
        }
