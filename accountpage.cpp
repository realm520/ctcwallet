    ...
    ui(new Ui::AccountPage)
{
    ...
    {
        if( CTC::getInstance()->addressMap.size() > 0)
        {
            accountName = CTC::getInstance()->addressMap.keys().at(0);
        }
        else  // 如果还没有账户
        {
...
void AccountPage::init()
{
    ...
    QString showName = CTC::getInstance()->addressMapValue(accountName).ownerAddress;
    address = showName;
...
void AccountPage::updateTransactionsList()
{
    ...
    {
        CTC::getInstance()->postRPC( toJsonFormat( "id_history_" + accountName + "_" + ui->assetComboBox->currentText(), "history", QStringList() << accountName << ui->assetComboBox->currentText() << "9999999" ));
    }
...
void AccountPage::updateAssetList()
{
    ...
    {
        qDebug() << "AccountPage::updateAssetList: " << key << ", " << CTC::getInstance()->ERC20TokenInfoMap.value(key).symbol;
...
void AccountPage::updateBalance()
{
    if( ui->assetComboBox->currentIndex() == 0)
    {
        if( !CTC::getInstance()->accountBalanceMap.contains(accountName))
        {
        ...
        AssetBalanceMap assetBalanceMap = CTC::getInstance()->accountBalanceMap.value(accountName);
        int assetId = CTC::getInstance()->getAssetId(ui->assetComboBox->currentText());
        if( assetId < 0)    return;
        unsigned long long amount = assetBalanceMap.value(assetId);
        AssetInfo info = CTC::getInstance()->assetInfoMap.value(assetId);
    ...
    else
    {
        ...
        QString accountAddress = CTC::getInstance()->addressMap.value(accountName).ownerAddress;
        unsigned long long amount = CTC::getInstance()->accountContractBalanceMap.value(accountAddress).value(contractAddress);
        ui->balanceLabel->setText( "<body><font style=\"font-size:18px\" color=#000000>" + getBigNumberString(amount,info.precision) + "</font><font style=\"font-size:12px\" color=#000000> " + ui->assetComboBox->currentText() + "</font></body>" );
        ui->balanceLabel->adjustSize();
    }
...
void AccountPage::jsonDataUpdated(QString id)
{
    if( id == "id_history_" + accountName + "_" + ui->assetComboBox->currentText())
    {
        ...
        CTC::getInstance()->parseTransactions(result,accountName + "_" + ui->assetComboBox->currentText());
...
void AccountPage::showNormalTransactions()
{
    ...
    TransactionsInfoVector vector = CTC::getInstance()->transactionsMap.value(accountName + "_" + ui->assetComboBox->currentText());
    ...
    for(int i = rowCount - 1; i > -1; i--)
    {
        ...
        // 对方账户
        ui->accountTransactionsTableWidget->setItem(i,3,new QTableWidgetItem( CTC::getInstance()->addressToName(detail.opposite)));

        // 金额
        AssetInfo assetInfo = CTC::getInstance()->assetInfoMap.value(detail.assetAmount.assetId);
...
//            result.opposite = QString::fromLocal8Bit("调用合约");
        ...
        else if( entry.fromAccount == accountName || entry.fromAccount == CTC::getInstance()->addressMap.value(accountName).ownerAddress)
        {
        ...
        {
            if( entry.toAccount == accountName || entry.toAccount == CTC::getInstance()->addressMap.value(accountName).ownerAddress)
            {
...
void AccountPage::showContractTransactions()
{
    ...
    ContractTransactionVector vector = CTC::getInstance()->accountContractTransactionMap.value(accountAddress + "-" + contractAddress);
    ...
    for(int i = rowCount - 1; i > -1; i--)
    {
        ...
        ui->accountTransactionsTableWidget->setItem(i,3,new QTableWidgetItem(CTC::getInstance()->addressToName(opposite)));
...
{
    QRCodeDialog qrcodeDialog(CTC::getInstance()->addressMapValue(accountName).ownerAddress);
