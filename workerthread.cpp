...
void WorkerThread::processRPC( QString cmd)
{
    ...
    if( CTC::getInstance()->stopping )    return;
...
//qDebug() << m_id << cmd;
    ...
    do
    {
        ...
        if( !socket->waitForReadyRead(10000) )
        {
            ...
            CTC::getInstance()->rpcReceivedOrNotMapSetValue(cmd, true);
...
//    qDebug() << m_id << receive;
    ...
    CTC::getInstance()->updateJsonDataMap(rpcId, result);
...
void WorkerThread::run()
{
    ...
    socket->connectToHost(QHostAddress("127.0.0.1"),CTC::getInstance()->currentPort);
