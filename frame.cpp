...
//                  );
    ...
    if( !CTC::getInstance()->configFile->contains("/settings/chainPath") )
    {
    ...
    {
        QString path = CTC::getInstance()->configFile->value("/settings/chainPath").toString();
        QDir dir(path);
        ...
        if( !dir.exists() || ( !dir2.exists() && !dir3.exists())  )  // 如果数据文件被删除了
        {
            ...
            CTC::getInstance()->configFile->clear();
            CTC::getInstance()->configFile->setValue("/settings/lockMinutes",5);
            CTC::getInstance()->lockMinutes     = 5;
            CTC::getInstance()->configFile->setValue("/settings/notAutoLock",false);
            CTC::getInstance()->notProduce      =  true;
            CTC::getInstance()->configFile->setValue("/settings/language","Simplified Chinese");
            CTC::getInstance()->language = "Simplified Chinese";
            CTC::getInstance()->minimizeToTray  = false;
            CTC::getInstance()->configFile->setValue("/settings/minimizeToTray",false);
            CTC::getInstance()->closeToMinimize = false;
            CTC::getInstance()->configFile->setValue("/settings/closeToMinimize",false);
        ...
        else
        {
            ...
            CTC::getInstance()->appDataPath = CTC::getInstance()->configFile->value("/settings/chainPath").toString();

            QStringList strList;
            strList << "--data-dir" << CTC::getInstance()->configFile->value("/settings/chainPath").toString()
                    << "--rpcuser" << "a" << "--rpcpassword" << "b" << "--rpcport" << QString::number(RPC_PORT) << "--server";
            if( CTC::getInstance()->configFile->value("/settings/resyncNextTime",false).toBool())
            {
                strList << "--resync-blockchain";

                QFile file(CTC::getInstance()->configFile->value("/settings/chainPath").toString() + "\\config.json");
                if(file.exists())   file.remove();
            }
            CTC::getInstance()->configFile->setValue("settings/resyncNextTime",false);
#ifdef WIN32
            CTC::getInstance()->startBlockChain(strList );

            if( CTC::getInstance()->proc->waitForStarted())
            {
                qDebug() << "launch ctc.exe succeeded";
            }
            else
            {
                qDebug() << "launch ctc.exe failed";
            }
#else
            CTC::getInstance()->proc->start("./ctc",strList );

            if( CTC::getInstance()->proc->waitForStarted())
            {
                qDebug() << "launch ctc succeeded";
            }
            else
            {
                qDebug() << "launch ctc failed";
            }
#endif
...
//    QTimer::singleShot(1000,mainPage,SLOT(show()));
    ...
    {
        timer->start(CTC::getInstance()->lockMinutes * 60000);
    }
...
void Frame::getAccountInfo()
{
    ...
    CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_list_my_addresses", "wallet_list_my_addresses", QStringList() << ""));

    CTC::getInstance()->postRPC( toJsonFormat( "id_balance", "balance", QStringList() << ""));

    CTC::getInstance()->updateAllContractBalance();

    CTC::getInstance()->collectContracts();
...
void Frame::showAccountPage(QString accountName)
{
    ...
    CTC::getInstance()->currentAccount = accountName;
    getAccountInfo();
...
void Frame::showLockPage()
{
    ...
    CTC::getInstance()->postRPC( toJsonFormat( "id_lock", "lock", QStringList() << "" ));
qDebug() << "lock ";
    DLOG_QT_WALLET_FUNCTION_END;
}
...
qDebug() << "unlock showCurrentDialog";
    CTC::getInstance()->showCurrentDialog();

    if( CTC::getInstance()->consoleWidget)
    {
        CTC::getInstance()->consoleWidget->showNormal();
        CTC::getInstance()->consoleWidget->activateWindow();
    }
...
void Frame::syncFinished()
{
    ...
    CTC::getInstance()->initWorkerThreadManager();

    if( CTC::getInstance()->contactsFile == NULL)
    {
        CTC::getInstance()->getContactsFile();
    }
...
void Frame::checkAlreadyQuit()
{
    ...
    if( CTC::getInstance()->proc->state() == QProcess::NotRunning)
    {
        qApp->quit();
    }
    else
...
void Frame::showTransferPage()
{
    closeCurrentPage();
    getAccountInfo();
    transferPage = new TransferPage(CTC::getInstance()->currentAccount,centralWidget);
    transferPage->setAttribute(Qt::WA_DeleteOnClose);
    connect(transferPage,SIGNAL(accountChanged(QString)),this,SLOT(showTransferPage(QString)));
    connect(transferPage,SIGNAL(showShadowWidget()),this,SLOT(shadowWidgetShow()));
    connect(transferPage,SIGNAL(hideShadowWidget()),this,SLOT(shadowWidgetHide()));
...
bool Frame::eventFilter(QObject* watched, QEvent* e)
{
    ...
    if( CTC::getInstance()->currentDialogVector.contains(  (QWidget*)watched) )
    {
        ...
        CTC::getInstance()->resetPosOfCurrentDialog();  // currentDialog 一起移动
...
void Frame::jsonDataUpdated(QString id)
{
    if( id == "id_open")
    {
        ...
        {
            // 其他情况视为钱包数据损坏
            QDir dir(CTC::getInstance()->appDataPath + "/wallets/wallet");
...
//            commonDialog.setText(QString::fromLocal8Bit("钱包数据损坏，请重新启动: ") + errorMessage );
    ...
    if( id == "id_wallet_list_my_addresses")
    {
        ...
        {
            QString addName = CTC::getInstance()->configFile->value("/accountInfo/" + key).toString();
            if( !accountList.contains( addName) && !addName.isEmpty())
            {
                // 如果config记录的账户钱包中没有 则清除config文件中记录以及内存中各map
                CTC::getInstance()->deleteAccountInConfigFile( addName);
                CTC::getInstance()->addressMapRemove( addName);
                CTC::getInstance()->balanceMapRemove( addName);
                CTC::getInstance()->registerMapRemove( addName);
            }
        }

        CTC::getInstance()->configFile->beginGroup("/accountInfo");
        keys = CTC::getInstance()->configFile->childKeys();

        foreach (QString key, keys)
        {
            QString addName = CTC::getInstance()->configFile->value(key).toString();
            CTC::getInstance()->balanceMapInsert(addName, CTC::getInstance()->getBalance( addName));
            CTC::getInstance()->registerMapInsert(addName, CTC::getInstance()->getRegisterTime( addName));
            CTC::getInstance()->addressMapInsert(addName, CTC::getInstance()->getAddress (addName));
        ...
        if( !accountList.isEmpty())    // 把config中没有记录的账号写入config中
        {
            for( int i = 0; i < accountList.size(); i++)
            {
                ...
                QString accountName = QString(QString::fromLocal8Bit("账户") + toThousandFigure(CTC::getInstance()->balanceMap.size() + 1));
                mutexForBalanceMap.unlock();
                CTC::getInstance()->configFile->setValue("/accountInfo/" + accountName, addName);
                CTC::getInstance()->balanceMapInsert(addName, CTC::getInstance()->getBalance( addName));
                CTC::getInstance()->registerMapInsert(addName, CTC::getInstance()->getRegisterTime( addName));
                CTC::getInstance()->addressMapInsert(addName, CTC::getInstance()->getAddress ( addName));
            }
    ...
    if( id == "id_lock")
    {
        ...
        if( result == "\"result\":null")
        {
            ...
            CTC::getInstance()->hideCurrentDialog();

            if( CTC::getInstance()->consoleWidget)
            {
                CTC::getInstance()->consoleWidget->hide();
            }
...
//        }
    ...
    {
        CTC::getInstance()->parseAssetInfo();

        updateAssets();

        CTC::getInstance()->updateAllToken();
    ...
    if( id.startsWith( "id_contract_call_offline_state+") )
    {
        ...
        if( !result.startsWith( "\"error\":") && result != "\"result\":\"NOT_INITED\"")
        {
            ...
            CTC::getInstance()->postRPC( toJsonFormat( "id_contract_call_offline_+tokenName+" + contractAddress, "contract_call_offline", QStringList() << contractAddress
                                                       << CTC::getInstance()->addressMap.keys().at(0) << "tokenName" << ""
                                                       ));

            CTC::getInstance()->postRPC( toJsonFormat( "id_contract_call_offline_+precision+" + contractAddress, "contract_call_offline", QStringList() << contractAddress
                                                       << CTC::getInstance()->addressMap.keys().at(0) << "precision" << ""
                                                       ));

            CTC::getInstance()->postRPC( toJsonFormat( "id_contract_call_offline_+admin+" + contractAddress, "contract_call_offline", QStringList() << contractAddress
                                                       << CTC::getInstance()->addressMap.keys().at(0) << "admin" << ""
                                                       ));

            CTC::getInstance()->postRPC( toJsonFormat( "id_contract_call_offline_+tokenSymbol+" + contractAddress, "contract_call_offline", QStringList() << contractAddress
                                                       << CTC::getInstance()->addressMap.keys().at(0) << "tokenSymbol" << ""
                                                       ));

            CTC::getInstance()->postRPC( toJsonFormat( "id_contract_call_offline_+totalSupply+" + contractAddress, "contract_call_offline", QStringList() << contractAddress
    ...
    if( id.startsWith( "id_contract_call_offline_+") )
    {
        ...
        if( result.startsWith("\"result\":"))
        {
            ...
            else if( func == "precision")
            {
                ...
                precision.remove('\"');
                CTC::getInstance()->ERC20TokenInfoMap[contractAddress].precision = precision.toDouble();
            }
            else if( func == "admin")
            {
            ...
            else if( func == "totalSupply")
            {
                ...
                totalSupply.remove('\"');
                CTC::getInstance()->ERC20TokenInfoMap[contractAddress].totalSupply = totalSupply.toDouble();
            }
        }
    ...
    if( id.startsWith( "id_contract_call_offline_balanceOf+") )
    {
        ...
        if( result.startsWith("\"result\":"))
        {
            ...
            if( CTC::getInstance()->accountContractBalanceMap.contains(accountAddress))
            {
                map = CTC::getInstance()->accountContractBalanceMap[accountAddress];
            }
            map.insert(contractAddress,balance);
            CTC::getInstance()->accountContractBalanceMap.insert(accountAddress,map);
        }
    ...
    if( id.startsWith( "id_blockchain_list_contract_transaction_history+") )
    {
        ...
        if( result.startsWith("\"result\":"))
        {
            ...
            if( !CTC::getInstance()->ERC20TokenInfoMap.contains(contractAddress)) return;
            CTC::getInstance()->ERC20TokenInfoMap[contractAddress].collectedBlockHeight = toBlockHeight.toInt();
                    ...
                    if( jsonObject.contains("result"))
                    {
                        ...
                        if( resultValue.isArray())
                        {
                            ...
                            for( int i = 0; i < resultArray.size(); i++)
                            {
                                ...
                                CTC::getInstance()->postRPC( toJsonFormat( "id_blockchain_get_pretty_contract_transaction" + trxId, "blockchain_get_pretty_contract_transaction",
                                                                             QStringList() << trxId
                                                                             ));
                            }
                        }
            ...
            if(json_error.error == QJsonParseError::NoError)
            {
                if( parse_doucment.isObject())
                {
                    ...
                    if( jsonObject.contains("result"))
                    {
                        ...
                        if( CTC::getInstance()->isMyAddress(contractTransaction.fromAddress))
                        {
                            QString key = contractTransaction.fromAddress + "-" + contractTransaction.contractAddress;
                            CTC::getInstance()->accountContractTransactionMap[key].append(contractTransaction);
                        }

                        if( CTC::getInstance()->isMyAddress(contractTransaction.toAddress) && contractTransaction.fromAddress != contractTransaction.toAddress)
                        {
    ...
    if( id == "id_wallet_get_transaction_fee" )
    {
        ...
        if( result.startsWith("\"result\":"))
        {
            ...
            CTC::getInstance()->transactionFee = amount.toLongLong();
...
void Frame::init()
{
    ...
    CTC::getInstance()->postRPC( toJsonFormat( "id_blockchain_list_pending_transactions", "blockchain_list_pending_transactions", QStringList() << "" ));

    CTC::getInstance()->postRPC(  toJsonFormat( "id_wallet_set_transaction_scanning", "wallet_set_transaction_scanning", QStringList() << "true" ) );

    CTC::getInstance()->postRPC( toJsonFormat( "id_blockchain_list_assets", "blockchain_list_assets", QStringList() << ""));

    CTC::getInstance()->postRPC( toJsonFormat( "id_wallet_get_transaction_fee", "wallet_get_transaction_fee", QStringList() << "" ));
