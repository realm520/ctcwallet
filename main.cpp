...
bool checkOnly()
{
    ...
    //  创建互斥量
    HANDLE m_hMutex  =  CreateMutex(NULL, FALSE,  L"CTCWALLET" );
    //  检查错误代码
...
bool checkOnly()
{
    const char filename[] = "/tmp/ctcwalletlockfile";
...
//    a.installTranslator(&translator);
    CTC::getInstance();  // 在frame创建前先创建实例，读取language
...
//    qInstallMessageHandler(outputMessage);  // 重定向qebug 到log.txt
    ...
    Frame frame;
    CTC::getInstance()->mainFrame = &frame;   // 一个全局的指向主窗口的指针
    frame.show();
