...
void CTC::deleteAccountInConfigFile(QString accountName)
{
    DLOG_QT_WALLET_FUNCTION_BEGIN;
...
TwoAddresses CTC::getAddress(QString name)
{
...
bool CTC::isMyAddress(QString address)
{
    if( address.isEmpty())
    {
        return false;
...
QString CTC::addressToName(QString address)
{
    foreach (QString key, addressMap.keys())
    {
...
QString CTC::getBalance(QString name)
{
    if( name.isEmpty())
    {
        return NULL;
    ...
    if( p != -1)  // 如果balance中存在
    {
        ...
        double amount = str.toDouble() / CTC::getInstance()->assetInfoMap.value(0).precision;
...
QString CTC::getRegisterTime(QString name)
{
    if( name.isEmpty())
    {
        return NULL;
...
void CTC::getSystemEnvironmentPath()
{
    ...
    foreach(str,environment)
    {
        ...
        {
            walletConfigPath = str.mid(5) + "/Library/Application Support/CTCWallet";
            appDataPath = walletConfigPath + "/chaindata";
            qDebug() << "appDataPath:" << appDataPath;
            break;
        }
    ...
    foreach(str,environment)
    {
        if (str.startsWith("HOME="))
        {
            ...
            walletConfigPath = str.mid(5) + "/CTCWallet";
            qDebug() << "appDataPath:" << appDataPath;
            break;
        }
    }
...
void CTC::changeToWalletConfigPath()
{
...
int CTC::getAssetId(QString symbol)
{
    int id = -1;
    foreach (int key, CTC::getInstance()->assetInfoMap.keys())
    {
        AssetInfo info = CTC::getInstance()->assetInfoMap.value(key);
...
void CTC::parseTransactions(QString result, QString accountName)
{
...
void CTC::updateAccountContractBalance(QString accountAddress, QString contractAddress)
{
...
void CTC::updateAllContractBalance()
{
    ...
    foreach (QString key, keys)
    {
        ...
        {
            updateAccountContractBalance(CTC::getInstance()->addressMap.value(key).ownerAddress,contract);
        }
    }
}

void CTC::updateERC20TokenInfo(QString contractAddress)
{
    if( addressMap.keys().size() < 1) return;
...
void CTC::collectContractTransactions(QString contractAddress)
{
...
void CTC::startBlockChain(QStringList &params)
{
    qDebug() << "ctc.exe " << params.join(" ");
    this->proc->start("ctc.exe", params);
}

void CTC::updateJsonDataMap(QString id, QString data)
{
...
QString CTC::jsonDataValue(QString id)
{
...
double CTC::getPendingAmount(QString name)
{
    mutexForConfigFile.lock();
...
QString CTC::getPendingInfo(QString id)
{
    mutexForConfigFile.lock();
    if( !pendingFile->open(QIODevice::ReadOnly))
    {
...
int CTC::getDelegateIndex(QString delegateName)
{
    mutexForDelegateList.lock();
    QStringList singleList = CTC::getInstance()->delegateList.filter( "\"name\":\"" + delegateName + "\"");
...
QString CTC::registerMapValue(QString key)
{
...
void CTC::registerMapInsert(QString key, QString value)
{
    mutexForRegisterMap.lock();
...
int CTC::registerMapRemove(QString key)
{
    mutexForRegisterMap.lock();
...
QString CTC::balanceMapValue(QString key)
{
...
void CTC::balanceMapInsert(QString key, QString value)
{
    mutexForBalanceMap.lock();
...
int CTC::balanceMapRemove(QString key)
{
    mutexForBalanceMap.lock();
...
TwoAddresses CTC::addressMapValue(QString key)
{
...
void CTC::addressMapInsert(QString key, TwoAddresses value)
{
    mutexForAddressMap.lock();
...
int CTC::addressMapRemove(QString key)
{
    mutexForAddressMap.lock();
...
bool CTC::rpcReceivedOrNotMapValue(QString key)
{
...
void CTC::rpcReceivedOrNotMapSetValue(QString key, bool received)
{
    mutexForRpcReceiveOrNot.lock();
...
void CTC::appendCurrentDialogVector( QWidget * w)
{
    currentDialogVector.append(w);
}

void CTC::removeCurrentDialogVector( QWidget * w)
{
    currentDialogVector.removeAll(w);
}

void CTC::hideCurrentDialog()
{
...
void CTC::resetPosOfCurrentDialog()
{
    foreach (QWidget* w, currentDialogVector)
    {
...
void CTC::initWorkerThreadManager()
{
...
void CTC::destroyWorkerThreadManager()
{
...
{
    mutexForAddressMap.lock();
    for (QMap<QString,TwoAddresses>::const_iterator i = CTC::getInstance()->addressMap.constBegin(); i != CTC::getInstance()->addressMap.constEnd(); ++i)
    {
        if(i.key() == strName)
        {
            mutexForAddressMap.unlock();
