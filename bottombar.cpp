...
//    DLOG_QT_WALLET_FUNCTION_BEGIN;
    ...
    QString result = CTC::getInstance()->jsonDataValue("id_info");
...
//        int numToSync = seconds.toInt()/ 10;
        ...
        CTC::getInstance()->currentBlockHeight = num.toInt();
        ...
        if( CTC::getInstance()->needToScan && scanPercent > 0.99999)
        {
            CTC::getInstance()->postRPC( toJsonFormat( "id_scan", "scan", QStringList() << "0"));
            CTC::getInstance()->needToScan = false;
        }
...
{
    CTC::getInstance()->postRPC( toJsonFormat( "id_info", "info", QStringList() << ""));
