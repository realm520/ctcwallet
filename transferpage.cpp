...
//    if( ui->feeLineEdit->text().toDouble() < 0.01)
//    {
//        CommonDialog tipDialog(CommonDialog::OkOnly);
    ...
    if( yOrN)
    {
        if( !ui->sendtoLineEdit->text().isEmpty())
        {
            ...
            else
            {
                ...
                CTC::getInstance()->postRPC( toJsonFormat( "id_contract_call_transfer+" + contractAddress + "+" + accountAddress, "contract_call",
...
void TransferPage::on_amountLineEdit_textChanged(const QString &arg1)
{
    ...
    QString strBalanceTemp = CTC::getInstance()->balanceMapValue(accountName).remove(",");
    ...
    else
    {
        ...
        QString accountAddress = CTC::getInstance()->addressMap.value(ui->accountComboBox->currentText()).ownerAddress;
        double balance;
        balance = CTC::getInstance()->accountContractBalanceMap.value(accountAddress).value(contractAddress);
...
{
    if( !CTC::getInstance()->contactsFile->open(QIODevice::ReadOnly))
    {
        qDebug() << "contact.dat not exist";
        contactsList.clear();
        return;
    }
    QString str = QByteArray::fromBase64( CTC::getInstance()->contactsFile->readAll());
...
void TransferPage::getBalance()
{
    ...
    if( assetIndex <= 0)
    {
        ...
        AssetBalanceMap map = CTC::getInstance()->accountBalanceMap.value(ui->accountComboBox->currentText());
...
qDebug() << id << result;
        if( result.mid(0,18) == "\"result\":{\"index\":")             // 成功
        {
            ...
            CTC::getInstance()->configFile->setValue("/recordId/" + recordId , 0);
