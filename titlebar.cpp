    ...
    ui(new Ui::TitleBar)
{
    ...
    QPalette palette;
    palette.setBrush(QPalette::Background, QBrush(QColor(47, 94, 178),Qt::SolidPattern));
    setPalette(palette);
...
void TitleBar::on_minBtn_clicked()
{
    ...
    if( CTC::getInstance()->minimizeToTray)
    {
        emit tray();
    ...
    {  
        ...
        if( CTC::getInstance()->consoleWidget)
        {
            CTC::getInstance()->consoleWidget->showMinimized();
        }
    }
...
void TitleBar::on_closeBtn_clicked()
{
    ...
    if( CTC::getInstance()->closeToMinimize)
    {
        emit tray();
    }
    else
    {
        ...
        commonDialog.setText( tr( "Sure to close Wallet?"));
...
void TitleBar::jsonDataUpdated(QString id)
{
    ...
    {
        QString pendingTransactions = CTC::getInstance()->jsonDataValue(id);
        // 查询一遍 config中记录的交易ID
        ...
        {
            if( CTC::getInstance()->configFile->value("/recordId/" + key).toInt() == 2)
            {
                // 失效的交易
                numOfNews++;
                continue;
            ...
            {
                CTC::getInstance()->postRPC( toJsonFormat( "id_blockchain_get_transaction_" + key, "blockchain_get_transaction", QStringList() << key  ));
            }

            if( CTC::getInstance()->configFile->value("/recordId/" + key).toInt() == 1)
            {
                numOfNews++;
            }
        }
...
//        }
    ...
    if( id.startsWith("id_blockchain_get_transaction"))
    {
        ...
        if( result.mid(0,22).contains("exception") || result.mid(0,22).contains("error"))
        {
            ...
            CTC::getInstance()->configFile->setValue("/recordId/" + id.right(40), 2);
        ...
        else   //  如果已经被打包进区块，则将config中记录置为1
        {
            ...
            CTC::getInstance()->configFile->setValue("/recordId/" + id.right(40), 1);
...
//    rpcThread->start();
    ...
    mutexForConfigFile.lock();
    CTC::getInstance()->configFile->beginGroup("/applyingForDelegateAccount");
    QStringList keys = CTC::getInstance()->configFile->childKeys();
    CTC::getInstance()->configFile->endGroup();
    foreach (QString key, keys)
    {
        // 如果申请代理的recordId 被删除了 或者被确认了（=1）或者失效了（=2） 则 删除applyingForDelegateAccount的记录
        if( !CTC::getInstance()->configFile->contains("/recordId/" + CTC::getInstance()->configFile->value("/applyingForDelegateAccount/" + key).toString())
            ||  CTC::getInstance()->configFile->value("/recordId/" + CTC::getInstance()->configFile->value("/applyingForDelegateAccount/" + key).toString()).toInt() != 0 )
        {
            CTC::getInstance()->configFile->remove("/applyingForDelegateAccount/" + key);
        }
    }

    CTC::getInstance()->configFile->beginGroup("/registeringAccount");
    keys = CTC::getInstance()->configFile->childKeys();
    CTC::getInstance()->configFile->endGroup();
    foreach (QString key, keys)
    {
        // 如果注册升级的recordId 被删除了 或者被确认了（=1）或者失效了（=2） 则 删除registeringAccount的记录
        if( !CTC::getInstance()->configFile->contains("/recordId/" + CTC::getInstance()->configFile->value("/registeringAccount/" + key).toString())
            ||  CTC::getInstance()->configFile->value("/recordId/" + CTC::getInstance()->configFile->value("/registeringAccount/" + key).toString()).toInt() != 0 )
        {
            CTC::getInstance()->configFile->remove("/registeringAccount/" + key);
        }
    }
...
//    consoleDialog.pop();
    ...
    if( CTC::getInstance()->consoleWidget)
    {
        CTC::getInstance()->consoleWidget->show();
        CTC::getInstance()->consoleWidget->activateWindow();
    }
    else
    {
        CTC::getInstance()->consoleWidget = new ConsoleWidget;
        CTC::getInstance()->consoleWidget->setAttribute(Qt::WA_DeleteOnClose);
        CTC::getInstance()->consoleWidget->show();
    }
