    ...
    ui(new Ui::SelectWalletPathWidget)
{
    ...
    ui->pathLineEdit->setText( CTC::getInstance()->appDataPath);
...
#ifdef WIN32
    CTC::getInstance()->startBlockChain(QStringList() << "--data-dir" << ui->pathLineEdit->text()
                                        << "--rpcuser" << "a" << "--rpcpassword" << "b" << "--rpcport" << QString::number(RPC_PORT) << "--server");
#else
    CTC::getInstance()->proc->start("./ctc",QStringList() << "--data-dir" << ui->pathLineEdit->text()
                                       << "--rpcuser" << "a" << "--rpcpassword" << "b" << "--rpcport" << QString::number( RPC_PORT) << "--server");
#endif
    if( CTC::getInstance()->proc->waitForStarted())
    {
        mutexForConfigFile.lock();
        CTC::getInstance()->configFile->setValue("/settings/chainPath", ui->pathLineEdit->text());
        mutexForConfigFile.unlock();
        CTC::getInstance()->appDataPath = ui->pathLineEdit->text();
        emit enter();
    }
    else
    {
